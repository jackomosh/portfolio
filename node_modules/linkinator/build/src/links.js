...
import schemaOrgUrlFields from './schema-org-url-fields.json' with { type: 'json'
};
...
export async function getLinks(source, baseUrl, checkCss = false) {
    ...
    const parser = new WritableStream({
        onopentag(tag, attributes) {
            const activeAnchor = tag === 'a' ? { text: '' } : undefined;
            ...
            if (tag === 'link' &&
                relTokens.length > 0 &&
                relTokens.every((rel) => ignoredLinkRelationships.has(rel.toLowerCase()))) {
                return;
            }
...
export function isValidFragment(fragment, validFragments) {
