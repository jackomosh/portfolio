...
let flags;
function compileRewritePattern(pattern) {
...
async function main() {
    ...
    if (flags.sitemap && flags.sitemapUrl) {
        throw new Error('The sitemap and sitemap-url flags cannot be used together.');
    }
    ...
    if (flags.urlRewriteExpressions) {
        options.urlRewriteExpressions = flags.urlRewriteExpressions.map((expression) => ({
