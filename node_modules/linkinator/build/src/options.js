...
/** @internal */
export async function findCommonPathRoot(paths, pathImplementation = path) {
    const filesystemRoot = pathImplementation.parse(paths[0]).root;
    const normalizedFilesystemRoot = filesystemRoot.toLowerCase();
    if (paths.some((filePath) => pathImplementation.parse(filePath).root.toLowerCase() !==
    ...
    if (!options.serverRoot && !isUrlType) {
        ...
        if (options.path.length > 1) {
            if (options.path.some((filePath) => path.isAbsolute(filePath))) {
                const absolutePaths = options.path.map((filePath) => path.resolve(filePath));
                const serverRoot = await findCommonPathRoot(absolutePaths);
                options.serverRoot = serverRoot;
                options.path = absolutePaths.map((filePath) => path.relative(serverRoot, filePath));
